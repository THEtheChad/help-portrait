...
export default async function(user: User) {
	const sanitized = sanitize(user.name)
	const formatted = sanitized.toLowerCase().replace(' ', '-')

	const root = CreateFolder(SESSION_DIR!, formatted)
	...
	try {
		...
		const info = root.chain(operation =>
			CreateFile(operation.meta.target, 'info.txt', JSON.stringify(user, null, 2)),
		)

		await Promise.all([raw, edited, info])
		console.log(`User account created: ${user.name}`)
	} catch (err) {
		if (err.code === 'EEXIST') {
			console.error(`User already exists: ${user.name}`)
		} else {
			throw err
		}
	}
